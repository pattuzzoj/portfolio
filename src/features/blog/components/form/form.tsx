...
export default function Form() {
  ...
  return (
    <form ref={menuRef} action="" className="group md:relative min-w-md" onFocus={() => setIsFocused(true)}>
      <input type="text" data-form-open={isFocused}className="w-full rounded-xl data-[form-open=true]:rounded-b-none p-2 bg-surface-subtle data-[form-open=true]:bg-surface outline-none" placeholder={t("form.search.placeholder")} value={search} onChange={e => setSearch(e.target.value)} />
      <div data-form-open={isFocused} className="w-full hidden data-[form-open=true]:flex flex-col gap-2 md:absolute md:top-full md:left-0 md:z-10 rounded-t-none rounded-b-xl bg-surface">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-2">
            <div className="relative">
              <Menu.Root onSelect={(details) => setCategory(details.value)}>
                ...
                <Menu.Content className="absolute top-full left-0 w-full z-10 flex flex-col p-1 border rounded-xl border-border bg-surface-subtle">
        ...
        <div className="flex flex-col justify-center items-center min-h-32">
          <Show when={posts.length > 0} fallback={(
            ...
            <a className="w-full p-2 hover:bg-accent hover:text-content-inverse text-center" href={url}>Ver todos os resultados</a>
          </Show>
        </div>
      </div>
    </form >
