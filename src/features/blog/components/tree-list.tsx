...
export default function HeadingTree(props: TreeProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex">
        <div id="track-line" className="relative w-0.5 bg-content-subtle">
          <div id="track-mark" className="absolute top-0 left-0 z-0 h-8 w-full bg-accent transition-all duration-300"></div>
        </div>
      ...
      <ol className="space-y-1">
        {props.tree?.map(({ depth, slug, text, children }) => (
          <li key={slug} className="space-y-1" style={{ "marginLeft": depth > 1 ? "16px" : "" }}>
            <a data-blog-menu-id={slug} data-blog-menu-active="false" className="block w-full px-2 py-1 text-left text-sm transition-all text-content-subtle hover:text-accent-hover data-[blog-menu-active=true]:text-accent data-[blog-menu-active=true]:font-semibold" href={`#${slug}`}>{text}</a>
