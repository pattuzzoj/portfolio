...
export default function ContactForm() {
  ...
  return (
    <form className="w-full flex flex-col gap-2 rounded-md p-4 bg-surface" action="/mail" method="POST" onSubmit={handleSubmit(onSubmit)}>
      ...
      <label className="flex flex-col gap-1">
        ...
        <textarea className="rounded-md border border-border p-2 bg-surface-subtle" placeholder={t("home:contact.form.fields.message.label")} rows={5} {...register("message")} />
        <Show when={Boolean(errors.message?.message)}>
          <p className="text-red-400">{errors.message?.message}</p>
        </Show>
      </label>
      <button className="flex justify-center items-center gap-2 rounded-md p-2 bg-accent text-content-inverse hover:bg-accent-hover disabled:bg-disabled" disabled={isSubmitting} type="submit">
